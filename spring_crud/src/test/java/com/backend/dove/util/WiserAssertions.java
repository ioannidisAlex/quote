...
public class WiserAssertions {
    ...
    public WiserAssertions withContent(Predicate<String> predicate) {
        findFirstOrElseThrow(m -> {
            ThrowingSupplier<String> contentAsString =
                    () -> ((String) getMimeMessage(m).getContent()).trim();
            return predicate.test(unchecked(contentAsString));
        }, assertionError("No message with content satisfying condition found!"));
        return this;
    }
    ...
    public WiserAssertions setVerbose(boolean verbose) {
